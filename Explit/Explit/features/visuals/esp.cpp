...
void c_esp::start()
{
	if (g_config.settings.visuals.esp.esp)
	{
		for (int i = 1; i <= g_interfaces.p_entity_list->get_highest_entity_index(); i++) {
			...
			if (const auto visible = g_interfaces.g_local_player->is_visible(p_entity); !(g_config.settings.visuals.esp.visible && !visible))
			{
				...
				if (g_config.settings.visuals.esp.enemies && p_entity->m_iteamnum() != g_interfaces.g_local_player->m_iteamnum() && p_entity->is_valid())
					draw_players(p_entity, box, visible);
...
void c_esp::draw_players(c_base_entity* p_player, const RECT box, const bool visible)
{
